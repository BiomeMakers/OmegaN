...
def build(cut):
    ...
    # COMPONENTE CONEXA MAYOR: sin esto lambda_2 = 0 y la coordenada de Fiedler
    # es un indicador de componente, no una coordenada suave. Se perdio al
    # reescribir sin networkx y contamino todas las corridas intermedias.
    ...
    if ncomp > 1:
        ...
        print(f"      [{ncomp} componentes; se queda la mayor: "
              f"{len(keep)} de {len(lab)} nodos]", flush=True)
    return A, nodes
